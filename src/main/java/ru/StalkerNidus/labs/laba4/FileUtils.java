    ...
    public static void saveWorld(File path, World world) throws IOException{
        try(BufferedWriter bw = new BufferedWriter(new FileWriter(path)))
        {
            ...
            for(Entity e : world.getEntities()) {
                ...
                if (e instanceof EntityPlayer) sb.append(";").append(((EntityPlayer) e).getNickname()).append(";").append(((EntityPlayer) e).getXp());
                sb.append("\n");
            }
            bw.write(sb.toString());
        }
    }
    public static World loadWorld(File path) throws IOException, SQLException {
        if (!path.exists()) return null;
        try(BufferedReader br = new BufferedReader(new FileReader(path)))
        {
            String s;
